...
\newpage
    ...
    \section{Engenharia de Fabricação}
        ...
        \subsection{Processos de Fabricação}
            \paragraph{Definição}Realização de atividades que modifiquem o produto com a finalidade de incrementar o valor pago pelo cliente. Estes procedimentos são dividos nas seguintes categorias:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item \textbf{Processos Mecânicos:} Modificações provocadas pela aplicação de tensões externas sobre os corpos, sendo classificados da seguinte maneira:
                        \begin{enumerate}[rightmargin = \leftmargin]
                            \item $\sigma < \sigma_{r}$, \texttt{Conformação Plástica:} Aplicação de forças sem remoção de material do corpo, não superando a tensão de ruptura $\sigma_{r}$, agindo na região plástica do material;
                                \begin{enumerate}[noitemsep]
                                    \item \textbf{Estampagem:} Corte ou dobra de chapas;
                                    \item \textbf{Extrusão:} Direta se o a pressão e a saída possuem o mesmo sentido e Indireta caso contrário;
                                    \item \textbf{Forjamento:} ;
                                    \item \textbf{Trefilação:} Tração do material, geralmente com excelente qualidade superficial e acabamento;
                                    \item \textbf{Laminação:} Menor redução, maior qualidade, pode ser realizada a quente ou a frio;
                                \end{enumerate}
                            ...
                            \item $\sigma > \sigma_{r}$, \texttt{Usinagem:} Aplicação de forças com remoção de material do corpo, superando a tensão de ruptura $\sigma_{r}$;
                        \end{enumerate}

                    \item \textbf{Processos Metalúrgicos:} Modificações relacionadas a altas temperaturas sobre os corpos, sendo classificados da seguinte maneira:
                        \begin{enumerate}[rightmargin = \leftmargin]
                            \item $T < T_{f}$, \texttt{Solidificação:} Aplicação de temperaturas que não alteram a fase do corpo, não superando a temperatura de fusão $T_{f}$;
                            \item $T > T_{f}$, \texttt{Metalurgia do Pó:} Aplicação de temperaturas que alteram a fase do corpo, superando a temperatura de fusão $T_{f}$;
                        \end{enumerate}

                    \item \textbf{Processos de Fundição:} Criação de corpos através da solidificação do material no interior de um molde, sendo classificados da seguinte maneira:
                        \begin{enumerate}[rightmargin = \leftmargin]
                            \item \texttt{Molde de Areia:} Utilização de areia para criação do molde, geralmente aplicados em pequena escala com as seguintes características:

                            \item \texttt{Molde de Casca:} Utilização de uma resina fenólica para criação do molde com as seguintes características:

                            \item \texttt{Molde Permanente:} Utilização de metal ou grafite para criação do molde com as seguintes características:

                            \item \texttt{Coquilha:} Utilização de metal para criação do molde onde o metal é vazado sob pressão com as seguintes características:

                            \item \texttt{Cera Perdida:} Utilização de cera para criação do molde que se desintegra após confecção com as seguintes características:
                        \end{enumerate}
                \end{enumerate}
                ...
                \begin{table}[H]
                    ...
                    \begin{tabular}[]{c | c c c c c}\hline
                        ...
                        $T_{f}$        & Alta    & Alta   & Alta     & Baixa    &   \\
                        Acabamento     & Baixo   &        & Alto     & Alta     &   \\
                        Formas         & Simples &        & Complexa & Complexa &   \\
