...
\newpage
    ...
    \section{Inferência}
        ...
        \subsection{Intervalo de Confiança 1 População}
        ...
        \subsection{Hipótese}
            \paragraph{Definição}Afirmações sobre um parâmetro de uma população que deverá ser testado. Desta maneira, será necessário estabelecer procedimentos a serem seguidos como explicados abaixo:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item \textbf{Suposições:} Normalmente assume-se que os dados obtidos do experimento foram produzidos através de um processo de aleatorização;

                    \item \textbf{Hipóteses:} Afirmação sobre um parâmetro populacional desejado que deverá ser definido antes da realização do experimento;
                        \begin{enumerate}[noitemsep]
                            \item $H_{0}$, \texttt{Hipótese Nula:} Parâmetro populacional assume um dado valor;
                            \item $H_{A}$, \texttt{Hipótese Alternativa:} Parâmetro populacional assume valores diferentes de $H_{0}$;
                        \end{enumerate}

                    \item \textbf{Estatísticas:} Comparação entre os valores encontradas na população após experimentação e os esperados pela \textbf{Hipótese}, obtido pela seguinte equação:
                        \begin{equation*}
                            z = \frac{\hat{p} - p_{0}}{\sqrt[]{\frac{p_0(1-p_{0})}{n}}} \sim
                            N(0,1)
                            \quad\text{onde}\quad
                            \begin{cases}
                                np_{0} \ge 10\\
                    ...
                    Onde:
                        \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                            \item $\hat{p}$ \texttt{Proporção Amostral:} Obtida pela seguinte equação:
                                \begin{equation*}
                                    \hat{p} \sim N\left( p, \frac{p(1-p)}{n}\right)
                                \end{equation*} 
                            \item $p_{0}$ \texttt{Proporção Populacional};
                        \end{enumerate}

                    \item \textbf{p-valor:} Probabilidade que resume a possibilidade de que a \texttt{Hipótese Nula} seja verdadeira, obtido pela seguinte equação:
                        \begin{equation}
                            \boxed{
                                \text{\textbf{p-valor}} = 
                                \begin{cases}
                                    P(|Z|\ge|z|) & \hat{p}\neq p_{0}\quad\text{Bilateral}\\
                                    P( Z \le z ) & \hat{p}   < p_{0}\quad\text{Unilateral à Esquerda}\\
                                    P( Z \ge z ) & \hat{p}   > p_{0}\quad\text{Unilateral à Direita}\\
                                \end{cases}
                            }
                        \end{equation}

                    \item \textbf{Conclusão:} Análise baseada no \textbf{p-valor} com base em um \textbf{Nível de Significância} $\alpha = 0.05$, representando uma constante usual que poderá ser interpretada da seguinte maneira:
                        \begin{enumerate}[noitemsep]
                            \item Se $\text{\textbf{p-valor}} \le \alpha$: rejeita-se $H_{0}$, há evidência contra a \texttt{Hipótese Nula};
                            \item Se $\text{\textbf{p-valor}}   > \alpha$: não rejeita-se $H_{0}$, não há evidência contra a \texttt{Hipótese Nula};
                        \end{enumerate}
                \end{enumerate}

        \subsection{Erros}
            \paragraph{Definição}Quando realiza-se um teste de hipóteses haveram diferentes combinações entre o que de fato e verdadeiro e o que foi obtido através da análise. Alguns resultados possíveis são demonstradados na seguinte tabela:
                \begin{table}[H]
                    \centering
                    \begin{tabular}[]{c|cc}\hline
                        Decisão          & $H_{0}$ Verdadeira & $H_{0}$ Falsa\\\hline
                        Rejeitar $H_{0}$ & Erro do Tipo I     & -\\
                        Aceitar $H_{0}$  & -                  & Erro do Tipo II\\\hline
                    \end{tabular}
                    \caption{Classificação Erros}
                \end{table}\noindent
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item \textbf{Erro do Tipo I:} Normalmente atribuído como \textbf{Falso Positivo}, apresentando a seguinte probabilidade de ocorrer, também nomeada \textbf{Nível de Significância}:
                        \begin{equation}
                            \alpha = P(\text{Erro do Tipo I})
                        \end{equation}

                    \item \textbf{Erro do Tipo II:} Normalmente atribuído como \textbf{Falso Negativo}, apresentando a seguinte probabilidade de ocorrer:
                        \begin{equation}
                            \beta = P(\text{Erro do Tipo II})
                        \end{equation}
                    Idealmente deseja-se que estas probabilidades sejam baixas, indicando hipóteses confiáveis. Entretanto, à medida que diminui-se $\alpha$, $\beta$ tende a aumentar. Desta maneira, estipula-se usualmente que $\alpha = 5\%$ para que os resultados sejam adequados.
                \end{enumerate}

        \subsection{Intervalo de Confiança 2 População}
            \paragraph{Definição}Representa relação entre uma variável dentro de duas populações independentes, 1 e 2, que possuem respectivamente $\mu_{1}$ e $\mu_{2}$ como média populacional, $\sigma_{1}^{2}$ e $\sigma_{2}^{2}$ como variância e $n$ e $m$ de tamanho. Assim um intervalo a partir de uma margem de erro, poderá contemplar uma porcentagem do resultado real será descrito pela seguinte equação:
                \begin{equation}
                    \boxed{
                        IC (\mu_{1} - \mu_{2}, 1-\alpha) = 
                        \left[
                            (\bar{x} - \bar{y}) - z_{\alpha/2}\sqrt{\frac{\sigma_{1}^{2}}{n} + \frac{\sigma_{2}^{2}}{m}};
                            (\bar{x} - \bar{y}) + z_{\alpha/2}\sqrt{\frac{\sigma_{1}^{2}}{n} + \frac{\sigma_{2}^{2}}{m}}
                        \right]
                    }
                \end{equation}
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item $\mu_{1} - \mu_{2}$, \textbf{Diferença da Média:} Representa a relação entre a diferença as médias das populações;

                    \item $\bar{x} - \bar{y}$, \textbf{Diferença da Amostral:} Representa a relação entre a diferença as populações;
                        \begin{equation*}
                            \bar{X} \sim N \left(\mu_{1}, \frac{\sigma_{1}^{2}}{n}\right)
                            \qquad
                            \bar{Y} \sim N \left(\mu_{2}, \frac{\sigma_{2}^{2}}{m}\right)
                        \end{equation*}

                    \item $1-\alpha$, \textbf{Intervalo de Confiança:} Representa a margem de erro aceitável para variável, descrito como uma distribuição normal:
                        \begin{equation}
                            \boxed{
                                1-\alpha =
                                \begin{cases}
                        ...
                        \end{equation}
                \end{enumerate}
            Note que quando as variâncias populacionais forem desconhecidas, será necessário realizar as seguintes contas:
                \begin{equation}
                    \boxed{
                        IC (\mu_{1} - \mu_{2}, 1-\alpha) = 
                        \left[
                            (\bar{x} - \bar{y}) - t\sqrt{S_{p}^{2}\left(\frac{1}{n} + \frac{1}{m}\right)};
                            (\bar{x} - \bar{y}) + t\sqrt{S_{p}^{2}\left(\frac{1}{n} + \frac{1}{m}\right)}
                        \right]
                    }
                \end{equation}
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item $S_{p}^{2}$, \textbf{Variância:} Representa a variância ponderada entre as populações, obtido pela seguinte equação:
                        \begin{equation}
                            S_{p}^{2} = \frac{(n-1)S_{1}^{2} + (m-1)S_{2}^{2}}{n + m - 2}
                        \end{equation}

                    \item $1-\alpha$, \textbf{Intervalo de Confiança:} Representa a margem de erro aceitável para variável, descrito como uma distribuição t-student:
                        \begin{equation}
                            \boxed{
                                1-\alpha =
                                \begin{cases}
                                    90\%, & t = t_{n+m-2, \alpha/2}\\
                                    95\%, & t = t_{n+m-2, \alpha/2}\\
                                    99\%, & t = t_{n+m-2, \alpha/2}\\
                                \end{cases}
                            }
                        \end{equation}
                    Obtidos através de tabelas ou softwares especializados.
                \end{enumerate}
\end{document}