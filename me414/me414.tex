...
\newpage
    ...
    \section{Distribuição Amostral}
        ...
        \subsection{Teorema do Limite Central}
            \paragraph{Definição}
                \begin{displayquote}[][]
                    Considera-se uma amostra aleatória $X_{1}$, ..., $X_{n}$ coletada de uma população com média $\mu$ e variância $\sigma^{2}$, então a distribuição amostral de $\overline{X}$ aproxima-se de uma \textbf{Distribuição Normal} de média $\mu$ e variância $\frac{\sigma^{2}}{n}$ quando $n$ for suficientemente grande:
                        \begin{equation}
                            \boxed{
