...
\newpage
    ...
    \section{Análise Combinatória}
        ...
        \subsection{Arranjo}
            \paragraph{Definição}Suponha que tenhamos uma coleção $\Omega = \{ \omega_{1}, ..., \omega_{n} \}$ de $n$ finitos objetos que devem ser separadas em $r$ grupos, então:
                \begin{displayquote}[][]
                    Haverá $n$ opções disponíveis para a primeira escolha, na sequência, haverá $n-1$ opções disponíveis para a segunda escolha e assim suscetivamente até que reste $n-r$ objetos da coleção. Isso ocorre, pois o \textbf{Arranjo} permite separar elementos de uma coleção em grupos independente de sua ordem.
                \end{displayquote}
                \begin{equation}
                    \boxed{
                        A(n, r) = \frac{n!}{(n - r)!} = n \times (n - 1) \times \cdots \times (n - r + 1)
                    }
                \end{equation}
