    ...
    \section{Circuitos Trifásicos}
        ...
        \subsection{Sistemas Monofásicos}
            \begin{definition}
                Dispositivos alimentados por fontes de tensão com a mesma fase usualmente representados pela seguinte figura:
                    \begin{figure}[H]
                        ...
                        \begin{circuitikz}
                            ...
                            \draw
                            (0,-2)  coordinate (phaseB)
                                    to[sV=$v_{BN}$] (0,0) coordinate (neutro)
                                    to[sV=$v_{AN}$] (0,2) coordinate (phaseA)
                                    
                            (phaseA)to[short, -o] ++(2,0)
                                    node[right] {$A$}
                    
                            (neutro)to[short, *-o] ++(1.5,0)
                                    node[right] {$N$}
                    
                            (phaseB)to[short, -o] ++(2,0)
                                    node[right] {$B$};
                        \end{circuitikz} 
                ...
                Onde:
                    \begin{equation}
                        \boxed{v_{AN}(t)=V_{A}\phase{\omega^{\circ}}}
                        \qquad
                        \boxed{v_{BN}(t)=V_{B}\phase{\omega^{\circ}}}
                    \end{equation}
                Nota-se que, usualmente, as tensões apresentadas serão eficazes.
            \end{definition}\noindent
            Quando estes sistemas estiverem conectados a \textbf{Carga Equilibradas}, isto é, apresentam mesma impedância, não haverá corrente no ramo neutro.
        ...
        \subsection{Sistemas Trifásicos}
            \begin{definition}
                ...
                Onde:
                    \begin{equation}
                        \boxed{v_{AN}(t) = V_{P}\phase{0^{\circ}}}
                        \qquad
                        \boxed{v_{BN}(t) = V_{P}\phase{-120^{\circ}}}
                        \qquad
                        \boxed{v_{CN}(t) = V_{P}\phase{+120^{\circ}}}
                    \end{equation}
                Note que adota-se a primeiro nó como \textbf{referência} da fase zero, o segundo nó como fase \textbf{menos} $120^{\circ}$ e o terceiro nó como fase \textbf{mais} $120^{\circ}$ como representado no seguinte diagrama fasorial:
                    \begin{figure}[H]
                        \centering
                        \begin{tikzpicture}
                            \begin{axis}[
