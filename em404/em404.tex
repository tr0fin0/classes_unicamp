...
\newpage
    ...
    \section{Cinética Plana de Corpos Rígidos}
        ...
        \subsection{Trabalho e Energia}
            ...
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item $U$, \textbf{Impulso:} Causado pela presença de forças realizando trabalho ou momentos aplicados. Quando um momento constante for aplicado ao corpo rígido será necessário multiplicá-lo pela comprimento da trajetória para obter as equações necessárias.
                    ...
                    Essa expressão geral será simplificada a depender das situações e dos pontos avaliadas, os principais pontos são descritos a seguir:
                        \begin{enumerate}[rightmargin = \leftmargin]
                            \item \textbf{Translação}: Quando houver apenas translação do corpo rígido avaliado, aplica-se a seguinte equação:
                                \begin{equation}
                                    \boxed{
                            ...
                            \item \textbf{Rotação em Eixo Fixo}: Quando houver apenas rotação do corpo rígido avaliado entorno de um eixo fixo, aplica-se a seguinte equação:
                                \begin{equation}
                                    \boxed{
                                        T_{i} = \frac{1}{2}I_{O}\omega^{2}
                                    }
                                \end{equation}
                            Neste caso nota-se que será necessário aplicar o teorema de eixos paralelos para obter o momento inércia do corpo rígido entorno deste eixo.
                                
                            \item \textbf{Centro de Velocidade Nula}: Quando a energia cinética do corpo rígido for avaliada entorno do centro de velocidade nula haverá apenas rotação, aplica-se a seguinte equação:
                                \begin{equation}
                                    \boxed{
                                        T_{i} = \frac{1}{2}I_{C}\omega^{2}
                                    }
                                \end{equation}
                            Neste caso nota-se que se o corpo rígido gira sem deslizar ao longo de uma trajetória este terá o centro de velocidade nula no ponto tangente entre o corpo e a trajetória.
                        \end{enumerate}
