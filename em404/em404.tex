...
\newpage
    ...
    \section{Mecânica Analítica}
        ...
        \subsection{Princípio de D'Alembert}
            \paragraph{Definição}Considera-se que em um referencial inercial, se uma partícula de massa $m$ é submetida a uma força $\vec{F}$ está adiquire uma aceleração absoluta $\vec{a}$ de acordo com a segunda lei de Newton, rescrita da seguinte forma:
                \begin{equation}
                    \boxed{
                        \delta W = 
                        \sigma
                        \left(
                            \vec{F}_{i} - \vec{\dot{G}}_{i}
                        \right) \cdot \vec{r}_{i} = 0
                    }
                \end{equation}
\end{document}