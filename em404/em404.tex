            ...
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                    ...
                    Neste cenário a aceleração $\vec{\alpha}$ será normal a $\vec{\omega}$, onde:
                        \begin{enumerate}[noitemsep]
                            \item $\vec{\Omega}$, \texttt{Velocidade Angular de $\omega$}: Representa a velocidade angular com que o próprio vetor $\omega$ gira entorno do cone espacial, ou seja, considerando a figura \ref{figEixo}; a rotação $\omega_{1}$ do corpo rígido;
                        \end{enumerate}
                \end{enumerate}
\end{document}