...
\newpage
    ...
    \section{Cinética de Partículas}
        ...
        \subsection{2\textsuperscript{a} Lei de Newton}
            \paragraph{Definição}Modelagem de um corpo através de $n$ partículas, de massa $m_{i}$, limitadas por uma superfície fechada no espaço, sofrendo ação tanto de forças externas, $F_{i}$, quanto de forças internas, $f_{i}$, a partir de um sistema de coordenadas inercial, sendo desrita pela seguinte equação:
                \begin{equation}
            ...
            Entretanto, como todas as forças internas ocorrem aos pares de mesmas direção e módulo, porém com sentidos opostos estas se cancelam. Assim, considera-se, pelo \textbf{Princípio de Movimento do Centro de Massa}, a força e aceleração resultantes podem ser analisadas com relação ao centro de massa do corpo, obtendo as seguintes equações:
                \begin{equation}
                    \boxed{
                        \sum\vec{F_{x}} = m \overline{\vec{a_{x}}}
                    }
                    \hspace{10mm}
                    \boxed{
                        \sum\vec{F_{y}} = m \overline{\vec{a_{y}}}
                    }
                    \hspace{10mm}
                    \boxed{
                        \sum\vec{F_{z}} = m \overline{\vec{a_{z}}}
                    }
                \end{equation}

        \subsection{Trabalho e Energia}
            \paragraph{Definição}Modelagem de um corpo através de $n$ partículas, de massa $m_{i}$, limitadas por uma superfície fechada no espaço com $(U_{1-2})_{i} = \Delta T_{i}$, trabalho realizado tanto de forças externas, $F_{i}$, quanto de forças internas, $f_{i}$, a partir de um sistema de coordenadas inercial, sendo desrita pela seguinte equação:
                \begin{equation}
            ...
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                    \item $T_{i}$, \textbf{Energia Cinética};
                    \item $V_{i}$, \textbf{Energia Potencial};
                    \begin{enumerate}[noitemsep]
                        \item $V_{ig}$, \textbf{Gravitacional};
                        \item $V_{ie}$, \textbf{Elástica};
                    \end{enumerate}
                \end{enumerate}
