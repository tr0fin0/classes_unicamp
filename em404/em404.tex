    ...
    \section{Cinemática Plana de Corpos Rígidos}
        ...
        \subsection{Movimento Plano}
            \paragraph{Definição}Movimento em que todas as partes do corpo se movem em planos paralelos geralmente define-se o plano de movimento como aquele que contém o centro de massa do corpo, sendo classificados como mostrado a seguir:
                \begin{enumerate}[rightmargin = \leftmargin]
                    ...
                    \item \textbf{Rotação:} Qualquer movimento angular em torno de um eixo fixo, onde todas as partículas do corpo se deslocam em trajetórias circulares, girando do mesmo ângulo ao mesmo tempo, aplicando coordenadas normal e tangencial têm-se:
                        \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
        ...
        \subsection{Centro Instantâneo de Velocidade Nula}
            \paragraph{Definição}Ponto de intersecção entre o plano do movimento e o eixo normal na qual o corpo possui apenas rotação pura, neste ponto a velocidade instantaneamente será nula. Considere a seguinte figura como referência para o desenvolvimento da localização deste ponto:
                \begin{figure}[H]
                    ...
                    \begin{subfigure}[]{0.3\textwidth}
                        \centering
                        \includegraphics[height = 4cm]{ima0.png}
                        \caption{Centro Instantâneo Genérico}
                    \end{subfigure}
                    \begin{subfigure}[]{0.3\textwidth}
                        \centering
                        \includegraphics[height = 4cm]{ima2.png}
        ...
        \subsection{Movimento Relativo com Rotação de Eixos}
            \paragraph{Definição}Composição do movimento através de sistemas cinemáticos móveis, referentes ao ponto $B$, que possuem rotação com relação a um sistema inercial, fixado no ponto $O$. Considere a seguinte figura como referência desta configuração:
                \begin{figure}[H]
                    \centering
                    \includegraphics[height = 4cm]{ima3.png}
                    \caption{Representação Eixos Rotativos}
                \end{figure} \noindent
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                    \item \textbf{Direção:} Vetorialmente a posição do ponto $A$ pode ser descrito como a composição entre o movimento rotacional e translacional, representado pela seguinte equação:
                        \begin{equation}
                            \boxed{
                    ...
                    Onde:
                        \begin{enumerate}[rightmargin = \leftmargin]
                            \item \texttt{Direção Relativa:} Representa o vetor de posição de $A$ com relação a $B$, representado pela seguinte equação:
                                \begin{equation}
                                    \boxed{
                                        \vec{r}_{AB} = x\vec{i} + y\vec{j}
                                    }
                                \end{equation}
                            ...
                            \item \texttt{Derivadas Direcionais:} Vetores unitários $\vec{i}$ e $\vec{j}$ possuem derivadas no tempo, pois possuem rotação, como mostrado nas equações abaixo:
                                ...
                                \end{equation}\noindent
                            \end{enumerate}
                        Além disso, as seguintes identidades serão válidas:
                            \begin{table}[H]
                                \centering
                    ...
                    \item \textbf{Velocidade:} Vetorialmente a velocidade do ponto $A$ pode ser descrito como a composição entre o movimento rotacional,  translacional e relativo, representado pela seguinte equação:
                        \begin{equation}
                            \boxed{
                                \vec{v}_{A} = \vec{v}_{B} + \omega_{AB}\times\vec{r}_{AB} + \vec{v}_{r}
                            }
                        \end{equation}
                    Onde:
                        \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                            \item \texttt{Velocidade Devido a Rotação:} Representa o vetor de velocidade causada pela rotação de $AB$, representado pela seguinte equação:
                                \begin{equation}
                                    \boxed{
                                        \vec{v}_{A|B} = \omega_{AB}\times\vec{r}_{AB} + \vec{v}_{r}
                                    }
                                \end{equation}

                            \item $v_{r}$, \texttt{Velocidade Relativa:} Representa o vetor de velocidade de $A$ com relação a $B$.
                        \end{enumerate}

                    \item \textbf{Aceleração:} Vetorialmente a aceleração do ponto $A$ pode ser descrito como a composição entre o movimento rotacional, translacional, relativo e de \textbf{Coriolis}, representado pela seguinte equação:
                        \begin{equation}
                            \boxed{
                                \vec{a}_{A} = 
                                \vec{a}_{B} + 
                    ...
                    Onde:
                        \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                            \item \texttt{Aceleração Devido a Rotação Tangencial:} Representa o vetor de aceleração causada pela rotação de $AB$ tangencial a trajetória considerada a partir do \textbf{Centro de Velocidade Nula}, representado pela seguinte equação:
                                \begin{equation}
                                    \boxed{
                            ...
                            \item \texttt{Aceleração Devido a Rotação Normal:} Representa o vetor de aceleração causada pela rotação de $AB$ normal a trajetória considerada a partir do \textbf{Centro de Velocidade Nula}, representado pela seguinte equação:
                                \begin{equation}
                                    \boxed{
                            ...
                            \item \texttt{Aceleração de Coriolis:} Aceleração causada pela rotação percebida apenas por observadores no sistema rotativo, representado pela seguinte equação: 
                                \begin{equation}
                                    \boxed{
                            ...
                            \item $a_{r}$, \texttt{Aceleração de Relativa:} Representa o vetor de aceleração de $A$ com relação a $B$.
                        \end{enumerate}
                \end{enumerate}
\newpage
    ...
    \section{Cinética Plana de Corpos Rígidos}
        ...
        \subsection{Momento de Inércia da Massa}
            \paragraph{Definição}Propriedade constante relacionada a geometria do corpo rígido estudado, quantificando a resistência à variação na velocidade de rotação em torno do eixo $z$ a partir do centro de gravidade obtido pela seguinte equação:
                \begin{equation}
                    \boxed{
                ...
                \begin{figure}[H]
                    ...
                    \begin{subfigure}[t]{0.3\textwidth}
                        \centering
                        \includegraphics[height = 2.5cm]{ima5.png}
                        \caption{Definição Ponto Arbitrário}
                    \end{subfigure}
                    \begin{subfigure}[t]{0.3\textwidth}
                        \centering
                        \includegraphics[height = 3cm]{ima7.png}
                        \caption{Definição Eixo Fixo}
                    \end{subfigure}
                    \caption{Representação de Corpos Rígidos}
                \end{figure} \noindent
            Corpos possuem diferentes momentos de inércia de acordo com o eixo de rotação considerado. Estes momentos são tabelas de para várias geométrias e eixos.

        \subsection{Equações Gerais}
            \paragraph{Definição}Nesta abordagem retoma-se os resultados obtidos para cinética de sistemas de partículas, aplicando-os para o centro de gravidade um corpo rígido genérico com os seguintes resultados:
                \begin{equation}
                    \boxed{
                ...
                \end{equation}
            Nestas equações será necessário notar que os termos a direita representam o diagrama
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item \textbf{Centro de Gravidade:} Neste ponto o momento de inércia a quantidade de movimento angular são obtidos pelas seguintes equações:
                        \begin{equation}
                            \boxed{
                                \Sigma\vec{F} = m\vec{\overline{a}}
                            }
                            \qquad
                            \boxed{
                                \Sigma\vec{M}_{G} = \overline{I}\alpha
                            }
                        \end{equation}

                    \item \textbf{Ponto Arbitrário:} Neste ponto, tanto para movimento relativo quanto para corpos interligados, o momento de inércia a quantidade de movimento angular são obtidos pelas seguintes equações:
                        \begin{equation}
                            \boxed{
                                \Sigma\vec{F} = \Sigma m\vec{\overline{a}}
                            }
                            \qquad
                            \boxed{
                                \Sigma\vec{M}_{G} = 
                                \dot{\vec{H}}_{G} + \overline{\rho}\times m \vec{\overline{a}} = 
                                \overline{I}\alpha + m\overline{a}d
                            }
                        \end{equation}

                    \item \textbf{Eixo Fixo:} Neste ponto haverá forças de reação dos apoios e o momento de inércia a quantidade de movimento angular são obtidos pelas seguintes equações:
                        \begin{equation}
                            \boxed{
                                \Sigma\vec{F} = \Sigma m\vec{\overline{a}}
                            }
                            ...
                            \boxed{
                                \Sigma\vec{M}_{O} = 
                                \overline{I}_{O}\alpha
                            }
                            \qquad
                            \boxed{
                                \overline{I}_{O} = 
                                \overline{I} + m\overline{r}^{2}
                            }
                        \end{equation}

                        \item \textbf{Centro de Percursão:} Neste ponto o momento de todas forças será nulo obtidos pelas seguintes equações:
                            \begin{equation}
                                \boxed{
                                    \Sigma\vec{F} = \Sigma m\vec{\overline{a}}
                                }
                            ...
                            \begin{figure}[H]
                                ...
                                \caption{Definição Centro de Percursão}
                            \end{figure}\noindent
                        Onde:
                            \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                                \item $k_{O}$, \texttt{Raio de Giração};
                                \item $q$, \texttt{Distância:} Obtido pela seguinte equação:
                                    \begin{equation}
                                        \boxed{
                                            q = \frac{k^{2}_{O}}{\overline{r}}
                                        }
                                    \end{equation} 
                            \end{enumerate}
                \end{enumerate}
            ...
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                    \item $T$, \textbf{Energia Cinética:} Analisa-se conjuntamento a parcela gerada pela translação e pela rotação, obtendo:
                        \begin{equation}
                            \boxed{
                                T_{i} = \frac{1}{2} m\overline{v}^{2} + \frac{1}{2}I_{G}\omega^{2}
                            }
                        \end{equation}
                    Onde:
                        \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
