...
\newpage
    ...
    \section{Cinética Tridimensional}
        ...
        \subsection{Movimento Translacional}
            ...
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item $\vec{r}_{A}$, \textbf{Deslocamento}:
                        \begin{equation}
                            \boxed{
                                \vec{r}_{A} = \vec{r}_{B} + \vec{r}_{AB}
                            }
                        \end{equation}

                    \item $\vec{v}_{A}$, \textbf{Velocidade}:
                        \begin{equation}
                            \boxed{
                                \vec{v}_{A} = \vec{v}_{B}
                            }
                    ...
                    \item $\vec{a}_{A}$, \textbf{Aceleração}:
                        \begin{equation}
                            \boxed{
                                \vec{a}_{A} = \vec{a}_{B}
                            }
        ...
        \subsection{Movimento Rotacional em Eixo Fixo}
        ...
        \subsection{Eixo Instantâneo de Rotação}
            \paragraph{Introdução}Primeiramente será necessário analisar o comportamento de um corpo rígido em movimento no espaço, enfatizando suas rotações. Desta forma, considera-se as seguintes figuras que realizam operações idênticas em ordens distintas:
                \begin{figure}[H]
                    ...
                    \begin{subfigure}[t]{0.45\textwidth}
                        \centering
                        \includegraphics[height = 4cm]{ima17.png}
                        \caption{Rotação em $x$ e $y$}
                    \end{subfigure}
                    \begin{subfigure}[t]{0.45\textwidth}
                        \centering
                        \includegraphics[height = 4cm]{ima18.png}
            ...
            \paragraph{Definição}Considera-se um corpo rígido rotacionado entorno de dois eixos com um ponto fixo $O$, este gira instaneamente em torno do \textbf{Eixo Instantâneo de Rotação} que passa por $O$, como representado na figura abaixo:
                \begin{figure}[H]
                    ...
                    \caption{Representação Eixo Instantâneo de Rotação}
                \end{figure}\noindent
            ...
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                    \item $\vec{\alpha}$, \textbf{Aceleração Angular}: Representa a derivado no tempo da velocidade angular do corpo rígido, refletindo a variação tanto no módulo como na diração de $\vec{\omega}$. Caso o módulo $\omega$ da rotação do corpo entorno do eixo instantâneo de rotação for constante, a seguinte equação será válida:
                        \begin{equation}
                            \boxed{
                                \vec{\alpha} = \vec{\Omega}\times\vec{\omega}
                            }
                        \end{equation}
                    Neste cenário a aceleração $\vec{\alpha}$ será normal a $\vec{\omega}$, onde:
                        \begin{enumerate}[noitemsep]
                            \item $\vec{\Omega}$, \texttt{Velocidade Angular de $\omega$}: Representa a velocidade angular com que o próprio vetor $\omega$ gira entorno do cone espacial, ou seja, considerando a Figura; a rotação $\omega_{1}$ do corpo rígido;
                        \end{enumerate}
                \end{enumerate}
\end{document}