...
\newpage
    ...
    \section{Mecânica Analítica}
        ...
        \subsection{Equações de Lagrange}
            \paragraph{Definção}Equações obtidas a partir do \textbf{Princípio de d'Alembert}, relacionando as energias envolvidas no sistema denotadas por $L = T - V$ como demonstrado pela seguinte equação:
                \begin{equation}
                    \boxed{
            ...
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item $q_{i}$: Coordenadas generalizadas do sistema;
                    \item $Q'_{i}$: Forças generalizadas não deriváveis de uma função potencial;
                    \item Vínculos são ideias e Holônomos;
                    \item Referência inercial;
                \end{enumerate}
\end{document}