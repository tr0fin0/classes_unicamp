...
\newpage
        ...
        \subsection{Impulso e Quantidade de Movimento Angular}
            ...
            Onde:
                \begin{enumerate}[rightmargin = \leftmargin]
                    \item $I_{xx}$ $I_{yy}$ $I_{zz}$, \textbf{Momento de Inérica nos Eixos:} Representam os momentos de inércias dos corpos com relação aos respectivos eixos e representados pelas seguintes equações:
                        \begin{equation}
        ...
        \subsection{Equações Gerais}
            ...
            Quando $\vec{H}$ é expresso em termos dos componentes medidos em relação a um sistema móvel $x-y-z$ que possui velocidade angular $\vec{\Omega}$ então o momento será:
                \begin{equation}
                    \boxed{
                        \Sigma\vec{M} = 
                        (\dot{H}_{x} - H_{y}\Omega_{z} + H_{z}\Omega_{y})\vec{i} + 
                        (\dot{H}_{y} - H_{z}\Omega_{x} + H_{x}\Omega_{z})\vec{j} + 
                        (\dot{H}_{z} - H_{x}\Omega_{y} + H_{y}\Omega_{x})\vec{k}
                    }
                \end{equation}
                ...
                \begin{enumerate}[rightmargin = \leftmargin]
                    ...
                    Onde:
                        \begin{enumerate}[rightmargin = \leftmargin, noitemsep]
                            ...
                            \item $\vec{G}$, \texttt{Quantidade de Movimento Linear no Centro de Massa};

                            \item $\vec{\omega}$, \texttt{Velocidade Angular em Torno do Centro de Massa};
                    ...
                    Note que realiza-se o produto escalar entre os vetores envolvidos, desta maneira relembra-se sua execução através da seguinte equação:
                        \begin{equation}
                            \boxed{
                                \vec{a} \cdot \vec{b} = 
                                (a_{x} \cdot b_{x})\vec{i} + 
                                (a_{y} \cdot b_{y})\vec{j} + 
                                (a_{z} \cdot b_{z})\vec{k}
                            }
                        \end{equation}
