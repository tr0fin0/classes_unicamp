...
\newpage
    ...
    \section{Aplicações Diodo}
        \subsection{Modelagem}
            ...
            Onde: $V_{DD} = 5V$, $V_{T} = 26mV$, $R = 1000\Omega$ e $I_{S} = 10^{-14}A$. Desta maneira, utilizam-se as seguinte aproximações para seu funcionamento:
                ...
                \begin{enumerate}[noitemsep]
                    ...
                    Considera-se configuração similar a ideal que quando em polarização direta o diodo se comporta como uma fonte de tensão constante enquanto quando em polarização reversa o diodo se comporta como uma chave aberta.
                        \[
                            \begin{cases}
                                R_{D} \to \infty \hspace{2.5mm} I_{D} = 0, &
        ...
        \subsection{Retificadores de Onda}
            \paragraph{Definição}

                \begin{enumerate}[noitemsep]
                    \item \textbf{Retificador de Meia Onda Positiva:}\\
                        \begin{figure}[H]
                            \centering
                            \begin{circuitikz}
                                \ctikzset{component text=left}
                                \draw
                                (0,0) to[sV, l=$V$] (0,2)
                                (0,2) to[empty diode, l=$D$] (4,2)
                                (4,2) to[resistor, l=$R$] (4,0)
                                (4,0) -- (0,0);
                            \end{circuitikz} 
                            \caption{Retificador de Meia Onda Positiva}
                        \end{figure} \noindent

                    \item \textbf{Retificador de Meia Onda Negativo:}\\
                        \begin{figure}[H]
                            \centering
                            \begin{circuitikz}
                                \ctikzset{component text=left}
                                \draw
                                (0,0) to[sV, l=$V$] (0,2)
                                (0,2) to[resistor, l=$R$] (4,2)
                                (4,2) to[empty diode, l=$D$] (4,0)
                                (4,0) -- (0,0);
                            \end{circuitikz} 
                            \caption{Retificador de Meia Onda Negativa}
                        \end{figure} \noindent

                    \item \textbf{Retificador de Meia Onda Negativa Limitador:}\\
                        \begin{figure}[H]
                            \centering
                            \begin{circuitikz}
                                \ctikzset{component text=left}
                                \draw
                                (0,0) to[sV, l=$V_{1}$] (0,4)
                                (0,4) to[resistor, l=$R$] (4,4)
                                (4,4) to[empty diode, l=$D$] (4,2)
                                (4,2) to[battery1, l=$V_{B}$] (4,0)
                                (4,0) -- (0,0);
                            \end{circuitikz} 
                            \caption{Retificador Limitador}
                        \end{figure} \noindent
                \end{enumerate}
\end{document}