...
\newpage
    ...
    \section{Transistor Bipolar de Junção}
        ...
        \subsection{Transistores NPN}
            \paragraph{Definição}Transistores em que a região central será composta por uma região P, positivamente dopada, apresentando o seguinte comportamento:
                \begin{figure}[H]
                    \centering
                    \begin{circuitikz}
                        \ctikzset{component text=left}
                        ...
                        (myComp.C) node [anchor = south] {C}
                        (myComp.E) node [anchor = north] {E}
                        (myComp.E) to[open, v^>=$v_{CE}$] (myComp.C)
