    ...
    \end{multicols}
        ...
        \subsection{Distância de Hamming}
            \paragraph{Definição}Representa o número de posições nos quais os dígitos correspondentes \textbf{diferem} entre si, como representado abaixo:
                \begin{table}[H]
                    ...
                    \end{tabular}
                    \caption{Representação da Distância de Hamming}\label{table:Hamming}
                \end{table}\noindent
        ...
        \subsubsection{Deteção de Erro de \texttt{1 bit}}
            ...
            \paragraph{Aplicação}Adiciona-se um \texttt{bit} em qualquer palavra válida para que o número total de \texttt{bits 1} seja:
                ...
                \end{enumerate}
            Selecionado a paridade basta incluir \texttt{bit de paridade} adequado. Assim, a partir deste valor é possível detectar erros durante a transmissão do símbolo de acordo com a paridade da codificação.
\end{document}